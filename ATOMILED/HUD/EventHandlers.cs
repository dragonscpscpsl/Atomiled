...
namespace HudForAtomiled
{
    public static class EventHandlers
    {
        ...
        {
            int k;
            return player != null && Kills.TryGetValue(player.UserId, out k) ? k : 0;
        }
        ...
        private static void OnDied(DiedEventArgs ev)
        {
            ...
            {
                foreach (Player scp106 in Player.List.Where(pl => pl.Role.Type == RoleTypeId.Scp106))
                {
                    AddKill(scp106);
            ...
            if (ev.Attacker != null && ev.Attacker != ev.Player)
            {
                ...
                foreach (Player spec in Player.List)
                {
                    SpectatorRole srole = spec.Role as SpectatorRole;
                    if (srole != null && srole.SpectatedPlayer == ev.Attacker)
        ...
        private static void AddKill(Player killer)
        {
            ...
            int v;
            if (Kills.TryGetValue(killer.UserId, out v))
        ...
        {
            foreach (Player spec in Player.List.Where(p => p.Role is SpectatorRole))
            {
