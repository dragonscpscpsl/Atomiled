...
#pragma warning disable SA1310 // Field names should not contain underscore
    ...
    internal sealed class Updater
    {
        ...
        private HttpClient CreateHttpClient()
        {
            ...
            client.DefaultRequestHeaders.Add("User-Agent", $"Atomiled.Loader (https://github.com/dragonscpscpsl/Atomiled, {Assembly.GetExecutingAssembly().GetName().Version.ToString(3)})");
        ...
        private void Update(HttpClient client, NewVersion newVersion)
        {
            try
            {
                ...
                ProcessStartInfo startInfo = new()
                {
                    ...
                    CreateNoWindow = true,
                    Arguments = $"--exit {(Folder == "global" ? string.Empty : $"--target-port {Folder}")} --target-version {newVersion.Release.TagName} --appdata \"{Paths.AppData}\" --exiled \"{config.AtomiledDirectoryPath}\" {(newVersion.IsPrerelease ? "--pre-releases" : string.Empty)}",
